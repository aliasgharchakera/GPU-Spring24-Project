...
coords_x = ti.field(dtype=ti.float32, shape=chromosome_size)
coords_y = ti.field(dtype=ti.float32, shape=chromosome_size)
...
def calculate_fitness(i: int):
    ...
    for j in range(chromosome_size - 1):
        ...
        dx = coords_x[index_current] - coords_x[index_next]
        dy = coords_y[index_current] - coords_y[index_next]
    ...
    dx = coords_x[index_last] - coords_x[index_first]
    dy = coords_y[index_last] - coords_y[index_first]
    total_distance += ti.sqrt(dx * dx + dy * dy)
